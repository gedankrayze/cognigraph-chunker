...
**Dynamic programming** finds the globally optimal partition. Unlike greedy approaches that make local decisions at each boundary, the DP explores all valid partitions (subject to a maximum chunk size derived from the token budgets) and selects the one that maximizes the total alignment score across all chunks; the reported partition score is this total normalized by chunk count. The time complexity is O(n * max_blocks) where n is the number of blocks, making it practical for documents up to tens of thousands of blocks.
